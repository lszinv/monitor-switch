...
namespace handle_info {
  ...
  void GetMonitorDeviceIds() {
    ...
    for (auto& info : monitorInfos) {
        ...
        while (EnumDisplayDevicesW(info.deviceName.c_str(), deviceIndex, &dd, EDD_GET_DEVICE_INTERFACE_NAME)) {
            ...
            std::wcout << L"    State Flags: 0x" << std::hex << dd.StateFlags << std::dec << L"\n";

            if (dd.StateFlags & DISPLAY_DEVICE_ACTIVE) {
                info.deviceId = dd.DeviceID;
                std::wcout << L"    This is the active device. Setting as monitor's Device ID.\n";
                found = true;
                break;
            }
            deviceIndex++;
        ...
        if (found) {
            std::wcout << L"Final Device ID for this monitor: " << info.deviceId << L"\n\n";
        } else {
  ...
  void MatchWithSetupAPI() {
    ...
    HDEVINFO deviceInfoSet = SetupDiGetClassDevs(&GUID_DEVINTERFACE_MONITOR, NULL, NULL, DIGCF_PRESENT | DIGCF_DEVICEINTERFACE);
    ...
    for (DWORD i = 0; SetupDiEnumDeviceInterfaces(deviceInfoSet, NULL, &GUID_DEVINTERFACE_MONITOR, i, &interfaceData); i++) {
      ...
      PSP_DEVICE_INTERFACE_DETAIL_DATA pDetailData = reinterpret_cast<PSP_DEVICE_INTERFACE_DETAIL_DATA>(&detailDataBuffer[0]);
      pDetailData->cbSize = sizeof(SP_DEVICE_INTERFACE_DETAIL_DATA);

      if (SetupDiGetDeviceInterfaceDetail(deviceInfoSet, &interfaceData, pDetailData, requiredSize, NULL, NULL)) {

        int length = MultiByteToWideChar(CP_ACP, 0, pDetailData->DevicePath, -1, NULL, 0);
        std::wstring setupApiDeviceId(length, L'\0');
        MultiByteToWideChar(CP_ACP, 0, pDetailData->DevicePath, -1, &setupApiDeviceId[0], length);
        ...
        for (auto& info : monitorInfos) {
          if (_wcsicmp(info.deviceId.c_str(), setupApiDeviceId.c_str()) == 0) {
            std::wcout << L"Matched HMONITOR " << info.hMonitor
                       << L" with SetupAPI device: " << setupApiDeviceId << std::endl;
            break;
          }
        }
      }
